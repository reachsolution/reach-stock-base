...
const generateBanKNiftyList = (last_price) => {
    ...
    const getNextThursday = (fromDate,isExpiryPassed=false) => {
        ...
        let daystoThursday= isdayLessThanFriday(day) && !isExpiryPassed ?daysToThursday(day):daysToNextThursday(day);    
    ...
    const isExpiryNotFinalWeek =(expiryDate)=>{
        ...
        let nextThursdayfromExpiry;
        nextThursdayfromExpiry= getNextThursday(expiryDate,true)
    ...
    const generateBankNiftyNameMonth=(expiryDay,last_price,pe_ce)=> `BANKNIFTY${(expiryDay.format("YY"))}${expiryDay.format("MMM").toUpperCase()}${last_price}${pe_ce}`
    //BANKNIFTY19JUN31000CE
    // --------------------------------------------------------------------------------------------------------
   ...
   // ---------------------------------------------------------------------------------------------------------
    for (let index = 1; index <= 4; index++) {
        let bnName=isExpiryNotFinalWeek(expiryDay)?generateBankNiftyName:generateBankNiftyNameMonth
       
            let delta=optionPoints;
            bankNifty=addBN(bnName, expiryDay, last_price, delta, bankNifty, day);
            if(isNotWednesdayorThursday(moment())){
...
const login = ({ apiSecret = 'ab972y2euma463jyba8qi1mctey72a1k', postLogin }) => {
    ...
    let reqToken = 'Cppvqfpoj3akSj3gJE6i6TQEUgU78v73'
    console.log('kc', kc)
...
function addBN(generateBankNiftyName, expiryDay, last_price, delta, bankNifty, day) {
    let pe = generateBankNiftyName(expiryDay, last_price, 'PE');
    let ce = generateBankNiftyName(expiryDay, last_price + delta, 'CE');
    return [...bankNifty,{ pe, ce, diff: delta, total: 0, targetTotal: target[day].total }];
}
